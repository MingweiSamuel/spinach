...
#[tokio::test]
pub async fn test_min() {
    let ( mut write_pipe, mut reader_pipe ) = SplitPipe::create();

    let read_pipe = NullPipe::new();
    let read_pipe = DebugPipe::new("read_0", read_pipe);
    reader_pipe.push_move(read_pipe).await;

    // write_pipe.push_move(()).await;
}
...
//         let write_pipe = LatticePipe::<'_, MapUnion<HashMap<&'static str, Max<&'static str>>>, _>::new(HashMap::new(), write_pipe);
...
//         read_pipe.push_move(read_pipe_foo).await;
...
//         write_pipe.push_move(( "foo", "bar" )).await;
