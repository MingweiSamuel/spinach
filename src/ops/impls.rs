...
impl<T> SharedRefOp for NullOp<T> {
    ...
    fn push(&self, _item: &Self::Indomain) -> Self::Feedback {
        future::ready(())
    }
}
...
pub struct LatticeOp<F: Merge, P: ExclRefOp<Indomain = F::Domain>> {
    value: F::Domain,
    next_pipe: P,
}
impl<F: Merge, P: ExclRefOp<Indomain = F::Domain>> LatticeOp<F, P> {
...
impl<F: Merge, P: ExclRefOp<Indomain = F::Domain>> Op for LatticeOp<F, P> {
    type Indomain = F::Domain;
}
impl<F: Merge, P: ExclRefOp<Indomain = F::Domain>> ExclMoveOp for LatticeOp<F, P> {
    type Feedback = P::Feedback;
...
pub struct TeeOp<P0: Op, P1: Op<Indomain = P0::Indomain>> {
    pipe0: P0,
    pipe1: P1,
}
impl<P0: Op, P1: Op<Indomain = P0::Indomain>> TeeOp<P0, P1> {
...
impl<P0: Op, P1: Op<Indomain = P0::Indomain>> Op for TeeOp<P0, P1> {
    type Indomain = P0::Indomain;
}
impl<P0: SharedRefOp, P1: Op<Indomain = P0::Indomain> + SharedRefOp> SharedRefOp for TeeOp<P0, P1> {
    type Feedback = impl Future;
...
impl<P0: ExclRefOp, P1: Op<Indomain = P0::Indomain> + ExclRefOp> ExclRefOp for TeeOp<P0, P1> {
    type Feedback = impl Future;
...
pub struct MapFilterOp<T, F: for<'a> UnaryFn<&'a T, Output = Option<P::Indomain>>, P: Op> {
    mapfilter: F,
    next_pipe: P,
    _phantom: std::marker::PhantomData<T>,
}
impl<T, F: for<'a> UnaryFn<&'a T, Output = Option<P::Indomain>>, P: Op> MapFilterOp<T, F, P> {
...
impl<T, F: for<'a> UnaryFn<&'a T, Output = Option<P::Indomain>>, P: Op> Op for MapFilterOp<T, F, P> {
    type Indomain = T;
}
impl<T, F: for<'a> UnaryFn<&'a T, Output = Option<P::Indomain>>, P: SharedMoveOp> SharedRefOp for MapFilterOp<T, F, P> {
    type Feedback = impl Future;
...
impl<T, F: for<'a> UnaryFn<&'a T, Output = Option<P::Indomain>>, P: ExclMoveOp> ExclRefOp for MapFilterOp<T, F, P> {
    type Feedback = impl Future;
