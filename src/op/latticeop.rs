...
use crate::flow::*;
use crate::lattice::{ Lattice, HideRef };
...
impl<O: PushOp<Inflow = Rx<F>>, F: Lattice> PushOp for LatticeOp<O, F> {
...
impl<O: MovePullOp<Outflow = Df<F::Domain>>, F: Lattice> RefPullOp for LatticeOp<O, F> {
    fn poll_next<'a>(
        ...
        let hide = HideRef::new(&self.state);
        Poll::Ready(Some(hide))
        // Poll::Ready(Some(&self.state))
    }
}
impl<O: RefPushOp<Inflow = Rx<F>>, F: Lattice> MovePushOp for LatticeOp<O, F> {
    type Feedback = O::Feedback;

    fn push<'a>(&mut self, item: <Self::Inflow as Flow>::Domain) -> Self::Feedback {
        F::merge_in(&mut self.state, item);
        let hide = HideRef::new(&self.state);
        self.op.push(hide)
    }
